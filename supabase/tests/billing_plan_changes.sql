...
from actors x join public.billing_accounts a on a.owner_user_id=x.id cross join (values('counted_clients',51),('coach_seats',3),('active_workspaces',4),('published_packages',4)) d(dimension,quantity) where x.name='blocked';
...
select is((select public.resolve_account_capacity(x.id,a.id)#>>'{dimensions,1,included}' from actors x join public.billing_accounts a on a.owner_user_id=x.id where name='blocked'),'1','included seats never exceed admission ceiling');
