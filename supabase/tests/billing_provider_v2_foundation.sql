...
-- Deferred validators use their trusted owner after RPC return. Neither has a
-- direct EXECUTE grant, and both retain a fixed search path.
select ok(p.prosecdef=(p.oid=any(array['public.billing_v2_validate_item_set()'::regprocedure,'public.billing_v2_validate_supersession_target()'::regprocedure])) and p.proconfig @> array['search_path=pg_catalog, public'] and not has_function_privilege(r,p.oid,'EXECUTE'),r||' cannot execute '||p.proname)
