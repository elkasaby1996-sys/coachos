...
create temp table expiry_before_completion as select id,expected_expires_at from public.billing_checkout_attempts where operation_id='b0500000-0000-4000-8000-000000000004';
select throws_ok($$select public.complete_billing_checkout_attempt(id,environment,creation_lease_expires_at,'fake-ready','https://fake-store.lemonsqueezy.com/checkout/custom/test',date_trunc('second',expected_expires_at)-interval '1 millisecond') from public.billing_checkout_attempts where operation_id='b0500000-0000-4000-8000-000000000004'$$,'P0001','BILLING_CHECKOUT_CREATION_AMBIGUOUS','completion rejects previous expiry second');
select throws_ok($$select public.complete_billing_checkout_attempt(id,environment,creation_lease_expires_at,'fake-ready','https://fake-store.lemonsqueezy.com/checkout/custom/test',date_trunc('second',expected_expires_at)+interval '1 second') from public.billing_checkout_attempts where operation_id='b0500000-0000-4000-8000-000000000004'$$,'P0001','BILLING_CHECKOUT_CREATION_AMBIGUOUS','completion rejects next expiry second');
select throws_ok($$select public.complete_billing_checkout_attempt(id,environment,creation_lease_expires_at,'fake-ready','https://fake-store.lemonsqueezy.com/checkout/custom/test',null) from public.billing_checkout_attempts where operation_id='b0500000-0000-4000-8000-000000000004'$$,'P0001','BILLING_CHECKOUT_CREATION_AMBIGUOUS','completion rejects null expiry');
select is((select status from public.billing_checkout_attempts where operation_id='b0500000-0000-4000-8000-000000000004'),'creating','rejected completions leave attempt unchanged');
select lives_ok($$select public.complete_billing_checkout_attempt(id,environment,creation_lease_expires_at,'fake-ready','https://fake-store.lemonsqueezy.com/checkout/custom/test',date_trunc('second',expected_expires_at)) from public.billing_checkout_attempts where operation_id='b0500000-0000-4000-8000-000000000004'$$,'successful completion accepts whole-second provider timestamp');
select is((select provider_expires_at from public.billing_checkout_attempts where operation_id='b0500000-0000-4000-8000-000000000004'),(select date_trunc('second',expected_expires_at) from expiry_before_completion),'actual provider timestamp is retained');
select is((select expected_expires_at from public.billing_checkout_attempts where operation_id='b0500000-0000-4000-8000-000000000004'),(select expected_expires_at from expiry_before_completion),'expected timestamp remains unchanged');
select throws_ok($$update public.billing_checkout_attempts set provider_expires_at=null where operation_id='b0500000-0000-4000-8000-000000000004'$$,'23514',null,'ready constraint rejects null provider expiry');
select throws_ok($$update public.billing_checkout_attempts set provider_expires_at=date_trunc('second',expected_expires_at)-interval '1 millisecond' where operation_id='b0500000-0000-4000-8000-000000000004'$$,'23514',null,'ready constraint rejects previous expiry second');
select throws_ok($$update public.billing_checkout_attempts set provider_expires_at=date_trunc('second',expected_expires_at)+interval '1 second' where operation_id='b0500000-0000-4000-8000-000000000004'$$,'23514',null,'ready constraint rejects next expiry second');
select throws_ok($$update public.billing_checkout_attempts set provider_checkout_id=null where operation_id='b0500000-0000-4000-8000-000000000004'$$,'23514',null,'ready constraint still requires provider checkout identity');
select throws_ok($$update public.billing_checkout_attempts set expected_expires_at=expected_expires_at+interval '1 second' where operation_id='b0500000-0000-4000-8000-000000000004'$$,'P0001','BILLING_CHECKOUT_OPERATION_CONFLICT','expected expiry remains immutable');
select throws_ok($$delete from public.billing_checkout_attempts where operation_id='b0500000-0000-4000-8000-000000000004'$$,'P0001','Billing history cannot be deleted.','checkout history remains protected');
