...
async function optionalOpenWearablesGet(label: string, path: string) {
  try {
    return await openWearablesGet(path);
  } catch (error) {
...
function rowsFromPayload(payload: Record<string, unknown>): Array<Record<string, any>> {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if ((body.action === "authorize" || body.action === "sync") && !provider) {
    ...
    if (body.action === "disconnect") {
      const { data: settings } = await supabase
        ...
        .select("client_can_disconnect, data_retention_mode")
        .eq("workspace_id", client.workspace_id)
        .maybeSingle();
      if (settings && settings.client_can_disconnect === false) {
    ...
    if (body.action === "sync") {
      ...
      const dailyRows = rowsFromPayload(timeseries).map((row) => ({
        workspace_id: client.workspace_id,
        client_id: client.id,
        provider,
        metric_date:
          stringOrNull(row.metric_date ?? row.date ?? row.recorded_at)?.slice(0, 10) ??
          new Date().toISOString().slice(0, 10),
        steps: numberOrNull(row.steps),
        active_minutes: numberOrNull(row.active_minutes ?? row.activeMinutes),
        distance_meters: numberOrNull(row.distance_meters ?? row.distanceMeters),
        ...
        max_heart_rate_bpm: numberOrNull(row.max_heart_rate_bpm ?? row.maxHeartRate),
      ...
      const sleepRows = rowsFromPayload(sleep).map((row) => {
        ...
        return {
          ...
          sleep_date:
            stringOrNull(row.sleep_date ?? row.start_at ?? row.startAt ?? row.start_time)?.slice(0, 10) ??
            new Date().toISOString().slice(0, 10),
          ...
          duration_minutes:
            ...
            (() => {
              const seconds = numberOrNull(row.sleep_duration_seconds ?? row.duration_seconds);
          ...
          avg_hr_bpm: numberOrNull(row.avg_hr_bpm ?? row.averageHeartRate ?? row.avg_heart_rate_bpm),
          avg_hrv_ms: numberOrNull(row.avg_hrv_ms ?? row.averageHrv ?? row.avg_hrv_ms),
          avg_spo2_percent: numberOrNull(row.avg_spo2_percent ?? row.averageSpo2 ?? row.avg_spo2_percent),
          respiratory_rate: numberOrNull(row.respiratory_rate ?? row.respiratoryRate),
        };
      });
      const activityRows = rowsFromPayload(workouts).map((row) => ({
        workspace_id: client.workspace_id,
        client_id: client.id,
        provider,
        provider_record_id:
        ...
        start_at: stringOrNull(row.start_at ?? row.startAt ?? row.start_time),
        end_at: stringOrNull(row.end_at ?? row.endAt ?? row.end_time),
        duration_seconds: numberOrNull(row.duration_seconds ?? row.durationSeconds),
        distance_meters: numberOrNull(row.distance_meters ?? row.distanceMeters),
        calories_kcal: numberOrNull(row.calories_kcal ?? row.calories),
        avg_hr_bpm: numberOrNull(row.avg_hr_bpm ?? row.averageHeartRate ?? row.avg_heart_rate_bpm),
        max_hr_bpm: numberOrNull(row.max_hr_bpm ?? row.maxHeartRate ?? row.max_heart_rate_bpm),
        strain_score: numberOrNull(row.strain_score ?? row.strain),
        source_payload_ref: stringOrNull(row.source_payload_ref ?? row.sourcePayloadRef),
      }));
      const scoreRows = rowsFromPayload(scores).map((row) => {
        const scoreType = stringOrNull(row.score_type ?? row.type ?? row.category) ?? "score";
        return {
          workspace_id: client.workspace_id,
          client_id: client.id,
          provider,
      ...
      return jsonResponse({ ...syncPayload, recordsImported });
    }
