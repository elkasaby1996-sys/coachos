-- Guard cross-workspace lead reassignment behind explicit PT transfer confirmation.
-- This keeps reassignment PT-only, prevents silent moves, and enforces a start-over flow.

drop function if exists public.pt_hub_approve_lead(uuid, uuid, text);
...
begin
    ...
    if v_transfer_requested and not coalesce(p_allow_transfer, false) then
      raise exception
        using
          ...
          hint = 'Confirm transfer because workspace-related client data will be reset.';
    end if;
  end if;
  ...
  begin
    ...
    if not v_transfer_requested
       and v_target_client_id is null
    ...
    if not v_transfer_requested and v_target_client_id is null then
      select c.id
      into v_target_client_id
      from public.clients c
