...
set
  status = 'submitted',
  activated_at = null
where status = 'partially_activated'
...
begin
  ...
  set
    ...
    activated_at = case when v_next_status = 'completed' then activated_at else null end
  where id = v_row.id
  returning *
  into v_row;
...
drop function if exists public.partially_activate_workspace_client_onboarding(uuid, text);
