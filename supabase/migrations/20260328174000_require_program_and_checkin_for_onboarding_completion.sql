...
declare
  ...
  v_effective_program_template_id uuid;
  v_effective_program_applied_at timestamptz;
  v_effective_checkin_template_id uuid;
  v_effective_checkin_date date;
  v_effective_checkin_scheduled_at timestamptz;
  v_display_name text;
  v_goal text;
  v_injuries text;
  v_limitations text;
...
begin
  ...
  v_effective_program_template_id := coalesce(
    p_program_template_id,
    v_row.first_program_template_id
  );
  v_effective_program_applied_at := coalesce(
    v_program_started_at,
    v_row.first_program_applied_at
  );

  if v_effective_program_template_id is null
    or v_effective_program_applied_at is null then
    raise exception 'First program assignment is required before completion';
  end if;

  v_effective_checkin_template_id := coalesce(
    p_checkin_template_id,
    v_row.first_checkin_template_id
  );
  v_effective_checkin_date := coalesce(
    case
      when p_checkin_template_id is not null then v_checkin_start_date
      else null
    end,
    v_row.first_checkin_date
  );
  v_effective_checkin_scheduled_at := coalesce(
    v_checkin_scheduled_at,
    v_row.first_checkin_scheduled_at
  );

  if v_effective_checkin_template_id is null
    or v_effective_checkin_date is null
    or v_effective_checkin_scheduled_at is null then
    raise exception 'First check-in scheduling is required before completion';
  end if;
