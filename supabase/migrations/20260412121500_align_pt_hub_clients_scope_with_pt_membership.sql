...
begin
  ...
  if p_workspace_id is not null and not public.is_pt_workspace_member(p_workspace_id) then
...
begin
  ...
  from public.clients c
  join pt_workspaces pw
    on pw.id = c.workspace_id
  left join lateral public.client_operational_snapshot(c.id) ops
    on true;
end;
$$;
