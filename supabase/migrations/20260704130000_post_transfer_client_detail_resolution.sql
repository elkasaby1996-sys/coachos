-- PR-05.5D: Ensure post-transfer coach navigation can resolve the target client.
-- The workspace slug route resolves client detail by clients.url_key, so the
-- explicit transfer primitive must assign and return target route identity.

drop function if exists public.pt_transfer_client_relationship(uuid, uuid) cascade;
