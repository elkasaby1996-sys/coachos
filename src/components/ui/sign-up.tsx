...
export interface AuthComponentProps {
  ...
  onEmailPasswordSubmit: (payload: { email: string; password: string }) => Promise<AuthSubmitResult | void>;
  onGoogle?: () => Promise<AuthSubmitResult | void>;
  onGithub?: () => Promise<AuthSubmitResult | void>;
}

const glassButtonVariants = cva("relative isolate cursor-pointer rounded-full transition-all disabled:cursor-not-allowed disabled:opacity-60", {
  variants: {
    size: {
  ...
  ({ className, children, size, contentClassName, ...props }, ref) => {
    return (
      ...
      >
        ...
        <span className={cn("relative z-10 text-foreground", glassButtonTextVariants({ size }), contentClassName)}>{children}</span>
      </button>
    );
  },
);
...
const GoogleIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg {...props} viewBox="0 0 24 24" aria-hidden="true">
    <path fill="#EA4335" d="M12 10.2v3.9h5.5c-.2 1.3-1.5 3.8-5.5 3.8a6 6 0 0 1 0-12c2.3 0 3.9 1 4.8 1.9l3.3-3.2A10.6 10.6 0 0 0 12 1.5a10.5 10.5 0 0 0 0 21c6.1 0 10.2-4.3 10.2-10.3 0-.7-.1-1.3-.2-2H12Z" />
    <path fill="#34A853" d="M1.5 12c0 1.7.4 3.2 1.2 4.6l3.8-3a6.3 6.3 0 0 1 0-3.2l-3.8-3A10.4 10.4 0 0 0 1.5 12Z" />
    <path fill="#4A90E2" d="M12 22.5c2.9 0 5.4-1 7.2-2.7l-3.5-2.8c-1 .7-2.2 1.2-3.7 1.2a6 6 0 0 1-5.6-4l-3.8 2.9A10.5 10.5 0 0 0 12 22.5Z" />
    <path fill="#FBBC05" d="M6.4 14.2a6.3 6.3 0 0 1 0-4l-3.8-2.9a10.5 10.5 0 0 0 0 9.8l3.8-2.9Z" />
  </svg>
);
...
export function AuthComponent({
  ...
  const [busyAction, setBusyAction] = useState<"none" | "email" | "google" | "github">("none");
  ...
  const headerTitle = title ?? (isSignUp ? "Create your account" : "Welcome back");
  const headerSubtitle =
    subtitle ??
    (isSignUp
      ? "Create your CoachOS account and continue to workspace setup."
      : "Sign in to manage clients and training plans.");
  const mainCtaLabel = primaryLabel ?? (isSignUp ? "Create account" : "Sign in");

  const runAction = useCallback(
    async (action: "email" | "google" | "github", fn: () => Promise<AuthSubmitResult | void>) => {
  ...
  const onSubmitEmail = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    await runAction("email", () => onEmailPasswordSubmit({ email: email.trim(), password }));
  };
  ...
  const socialButtons = useMemo(() => {
    ...
    return (
      <div className={cn("grid w-full gap-3", onGoogle && onGithub ? "grid-cols-1 sm:grid-cols-2" : "grid-cols-1")}>
        {onGoogle ? (
          <GlassButton
            type="button"
          ...
          >
            <span className="inline-flex items-center gap-2">
              {busyAction === "google" ? <Loader2 className="h-4 w-4 animate-spin" /> : <GoogleIcon className="h-4 w-4" />}
        ...
        {onGithub ? (
          ...
          >
            <span className="inline-flex items-center gap-2">
              {busyAction === "github" ? <Loader2 className="h-4 w-4 animate-spin" /> : <GitHubIcon className="h-4 w-4" />}
  ...
  return (
    <AuthBackdrop brandName={brandName} logo={logo} contentClassName="max-w-[30rem]">
      <motion.div
        ref={cardRef}
      ...
      >
        <div className="mb-6 space-y-2 text-center">
          <h1 className="font-serif text-3xl tracking-tight text-foreground sm:text-4xl">{headerTitle}</h1>
        ...
        {socialButtons ? (
          <div className="my-5 flex items-center gap-3">
            ...
            <span className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">Or with email</span>
            <div className="h-px flex-1 bg-border" />
          </div>
        ) : null}
        ...
        <form onSubmit={onSubmitEmail} className="space-y-4">
          ...
          <label className="block text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            ...
            <div className="mt-1.5 flex items-center gap-2 rounded-full border border-border/70 bg-background/80 px-3 py-2.5">
              ...
              />
              <button type="button" onClick={() => setShowPassword((prev) => !prev)} className="text-muted-foreground hover:text-foreground">
          ...
          {notice ? (
            <div className="rounded-xl border border-border bg-muted/50 px-3 py-2 text-sm text-muted-foreground">{notice}</div>
          ) : null}
          ...
          <GlassButton type="submit" className="w-full" disabled={busyAction !== "none"}>
            <span className="inline-flex items-center gap-2">
