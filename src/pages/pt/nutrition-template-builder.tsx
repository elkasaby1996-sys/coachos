...
export function PtNutritionTemplateBuilderPage() {
  ...
  const activeDay = useMemo(
    () =>
      template?.days.find((day) => day.week_index === selectedWeek && day.day_of_week === selectedDay) ?? null,
    [template, selectedWeek, selectedDay]
  );

  const slots = useMemo(() => {
    const map = new Map<string, { id: string | null; components: any[]; meal_order: number }>();
  ...
  const invalidate = async () => {
    await Promise.all([
      queryClient.invalidateQueries({ queryKey: ["nutrition-template-v1", templateId] }),
  ...
  const ensureSlot = async (slotName: string) => {
    ...
    const nextOrder = slots.length ? Math.max(...slots.map((s) => s.meal_order)) + 1 : 1;
  ...
  const addComponent = async () => {
    ...
    const { error } = await supabase.from("nutrition_template_meal_components").insert({
  ...
  const removeComponent = async (componentId: string) => {
    const { error } = await supabase.from("nutrition_template_meal_components").delete().eq("id", componentId);
    if (error) {
      setErrorMessage(error.message);
      return;
    }
  ...
  const moveComponent = async (
    ...
    const list = (slot?.components ?? []).slice().sort((a, b) => a.sort_order - b.sort_order);
  ...
  if (!template) {
    return (
      <PageContainer className="max-w-screen-2xl">
        <EmptyState title="Program not found" description="This nutrition program is unavailable." />
      </PageContainer>
    );
  }
    ...
    <PageContainer className="max-w-screen-2xl space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Nutrition program builder: slots + meal components with macro details.</p>
        </div>
        <Button variant="secondary" onClick={() => navigate("/pt/nutrition-programs")}>Back</Button>
      </div>

      {errorMessage ? (
        <div className="rounded-lg border border-destructive/40 bg-destructive/10 px-3 py-2 text-sm text-destructive">{errorMessage}</div>
      ) : null}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <DashboardCard title="Meal Slots" subtitle="Breakfast/Lunch/Dinner/etc">
          <div className="space-y-3">
            {slots.map((slot) => {
              ...
              return (
                <div key={slot.slot} className="rounded-lg border border-border/60 bg-muted/20 p-3">
                  <div className="flex items-center justify-between gap-2">
                    <div>
                      <p className="font-medium">{slot.slot}</p>
                      <p className="text-xs text-muted-foreground">{slot.components.length} components</p>
                    </div>
                    <Button size="sm" variant="ghost" onClick={() => openAddComponent(slot.slot)}>
                      <Plus className="mr-1 h-3.5 w-3.5" />
                      Add
                    </Button>
                  </div>
                  <p className="mt-2 text-xs text-muted-foreground">
                    {Math.round(slotTotals.calories)} cals | {Math.round(slotTotals.protein_g)}p | {Math.round(slotTotals.carbs_g)}c | {Math.round(slotTotals.fat_g)}f
                  </p>
                </div>
              );
            })}
        ...
        <DashboardCard title="Components" subtitle="All components for selected day">
            ...
            <div className="space-y-3">
              {slots.map((slot) => (
                <div key={slot.slot} className="space-y-2">
                  <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{slot.slot}</p>
                  {slot.components.map((component) => (
                    <div key={component.id} className="rounded-lg border border-border/60 bg-muted/20 p-3">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground line-clamp-2">{component.recipe_text ?? "No recipe"}</p>
                        </div>
                        <div className="flex gap-1">
                          <Button size="icon" variant="ghost" onClick={() => moveComponent(slot.id ?? "", component.id, "up")}><ArrowUp className="h-3.5 w-3.5" /></Button>
                          <Button size="icon" variant="ghost" onClick={() => moveComponent(slot.id ?? "", component.id, "down")}><ArrowDown className="h-3.5 w-3.5" /></Button>
                          <Button size="icon" variant="ghost" onClick={() => removeComponent(component.id)}><Trash2 className="h-3.5 w-3.5" /></Button>
                        </div>
                      </div>
                      <div className="mt-2 grid grid-cols-4 gap-2 text-center text-xs">
                        <div><p className="text-muted-foreground">Cals</p><p>{component.calories ?? 0}</p></div>
                        <div><p className="text-muted-foreground">P</p><p>{component.protein_g ?? 0}</p></div>
                        <div><p className="text-muted-foreground">C</p><p>{component.carbs_g ?? 0}</p></div>
                        <div><p className="text-muted-foreground">F</p><p>{component.fat_g ?? 0}</p></div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="rounded-lg border border-border/60 bg-muted/20 p-3 text-xs">
                ...
                <p className="text-muted-foreground">
                  {Math.round(dayTotals.calories)} cals | {Math.round(dayTotals.protein_g)}p | {Math.round(dayTotals.carbs_g)}c | {Math.round(dayTotals.fat_g)}f
                </p>
              </div>
            </div>
          )}
      ...
      <Dialog open={componentOpen} onOpenChange={setComponentOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Component inside the selected slot meal program.</DialogDescription>
          </DialogHeader>
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            <Input className="sm:col-span-2" placeholder="Component name" value={componentName} onChange={(e) => setComponentName(e.target.value)} />
            <Input placeholder="Quantity" value={quantity} onChange={(e) => setQuantity(e.target.value)} />
            <Input placeholder="Unit (g/ml/pcs)" value={unit} onChange={(e) => setUnit(e.target.value)} />
            ...
            <textarea className="sm:col-span-2 min-h-[100px] rounded-lg border border-input bg-background px-3 py-2 text-sm" placeholder="Recipe text" value={recipeText} onChange={(e) => setRecipeText(e.target.value)} />
