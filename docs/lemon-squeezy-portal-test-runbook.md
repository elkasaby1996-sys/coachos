...
Portal compatibility regression coverage includes both purposes with legacy expires/signature, optional user, additional provider fields, different parameter orders, repeated fields, and absent/empty query strings. The original URL must be preserved exactly and no expiresAt derived. A provider-returned URL without a query is accepted under the same authenticated provider and canonical identity checks; RepSync does not infer authorization from query presence. Verify `/billing` and the resolved subscription payment-details path with or without a single trailing slash, and rejection of descendants, wrong subscription IDs, unrelated/encoded invalid paths, wrong hosts, HTTP, credentials, explicit ports (including :443) and fragments.

Verify that ownership RPC arguments still contain only the authenticated owner and server environment, that post-retrieval identity/ownership checks fail closed, and that browser-supplied routing/identity inputs remain rejected. Telemetry, evidence, storage/cache isolation and no-store headers must hold even when no signature/expiry query keys are present. Never record a capability or its query values. This compatibility fix requires no migration. Local fixtures establish the validator contract; only a separately authorized staging exercise can close CERT-PORTAL-001.
