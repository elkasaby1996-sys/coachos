...
Portal compatibility regression coverage includes `manage_billing` with and without the optional provider-supplied `user`, safe positive decimal provider-ID validation, duplicate/unknown/redirect parameter rejection, required signature, expiry and exact host/path checks. `update_payment_method` continues rejecting `user`. Verify that ownership RPC arguments still contain only the authenticated owner and server environment, and that telemetry, storage/cache isolation and no-store headers remain intact. Never record the signed capability or its `user` value. This URL-validation compatibility fix requires no migration.
