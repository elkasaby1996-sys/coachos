...
## 25. PR #194 CI readiness follow-up

The [smoke job for run 34687214720](https://github.com/elkasaby1996-sys/coachos/actions/runs/34687214720/job/103536352694?pr=194) ran correction head `cd89c7f8a1cc8949b9e3b63e8e9cdba3be153eb6` with four workers and zero retries: 112 passed, ten inherited skips, one failure. All commercial correction and portal cases passed. The only failure was `PayPal has support guidance without provider mutation`, inside fixture setup before any PayPal preview or provider mutation.

The trace places the `Change plan` assertion at monotonic 251248.798–256281.275 ms. `get_my_billing_plan_change_state` starts at 256136.766 ms and completes 49.607 ms later, approximately 95 ms before the assertion deadline. The failure snapshot already includes the button. The lazy page's bootstrap and canonical state arrival consumed the assertion's five-second budget; document navigation completion was not billing readiness.

A permanent regression holds the first real plan-state response for six seconds without changing its payload. Before the fixture correction it reproduces the identical missing-button assertion failure. The fixture now registers a listener before navigation, waits for that exact POST response and its body, checks HTTP success, and then starts the unchanged visibility assertion. The test still verifies PayPal support guidance and zero provider mutations. No production code, SQL, provider policy, CI worker count, retry setting, global timeout or skip condition changed.

The first local four-worker plan-change run passed the delayed PayPal case but encountered six later existing preview/display assertions under memory pressure (about 395 MB free physical memory at inspection). Those failures are retained in `%TEMP%/pr-price-08-ci-plan-change-after.log` and are not represented as a passing full four-worker suite. Local Studio/analytics were temporarily stopped to reduce resource pressure; the database/auth/API remained available. Before-fix reproduction is `%TEMP%/pr-price-08-ci-readiness-before.log`.

Follow-up verification: the delayed response reproduces the failure before the fix; after the fix, four concurrent delayed PayPal repetitions passed with four workers and zero retries (1.3 minutes), and all 12 plan-change scenarios passed with one worker and zero retries (1.9 minutes). Logs: `%TEMP%/pr-price-08-ci-paypal-stress.log` and `%TEMP%/pr-price-08-ci-plan-change-final.log`. Lint passed with the same three inherited warnings; format and TypeScript/Vite build passed. The earlier release pair remains evidence for the production implementation; it predates this test-only follow-up. A new GitHub CI run is still required to establish the hosted job result. No production file, migration, provider operation or remote Supabase state was changed for this follow-up.
