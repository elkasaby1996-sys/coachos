...
The manifest freezes 161 migrations from `20260326084615_baseline_schema.sql` through `20260913010000_public_commercial_catalogue_v2.sql`. Every filename and SHA-256 is recorded. Hashes normalize CRLF to LF so Windows and Linux checkouts agree; other changes fail. The complete directory must match, timestamp versions must be unique and sorted, and the expected last file must match. Historical SQL is unchanged by Phase A.
